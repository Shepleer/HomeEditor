...
private extension ErrorPlaceholderView {
    ...
    @objc func retryButtonDidPressed() {
        /// so here we could add reload, but our Parcer service will be always return one result, next line of code will be correct if we will fetch data from server
//        onPress?()
    }
}
